...
class ModuleView(ttk.Frame):
    def __init__(self, master, module_name, *args, **kwargs):
        super().__init__(master, *args, **kwargs)
        self.module_name = module_name
        self.columnconfigure(1, weight=1)
        label_style = ttk.Style()
        label_style.configure("Default",  # имя стиля
                              font="helvetica 14",  # шрифт
                              foreground="#004D40",  # цвет текста
                              padding=8,  # отступы
                              background="#663127")  # фоновый цвет

        ttk.Label(self, text=f"{module_name.capitalize()} - simple calculator").grid(row=0, column=0, columnspan=2, sticky="w")
        ...
        ttk.Button(self, text="Compute (module default)", command=self.on_compute).grid(row=3, column=0, columnspan=2, pady=6)
    ...
    def on_compute(self):
        ...
        b = self.b_var.get()
        try:
            svc = importlib.import_module(f"gui.services.{self.module_name}_service")
            result = getattr(svc, "compute_example")(a, b)
